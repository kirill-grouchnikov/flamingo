...
public class RibbonGalleryTestCase extends TestCase {
                            ...
                            SwingUtilities.invokeAndWait(new Runnable() {
                                public void run() {
                                    ...
                                    tested[fSel] = (selectedCount == 1)
                                            && gallery.getButtonGroup("Group 0").get(fSel)
                                                    .getActionModel().isSelected();
                                };
                            });
                        }
                    } catch (Throwable t) {
    ...
    public void testRemoval3() {
        this.ribbonBand.removeRibbonGalleryButtons(GALLERY_NAME, this.buttons[2][9],
                this.buttons[1][9]);
        assertEquals(this.gallery.getButtonGroupCount(), 4);
    ...
    public void testRemovalAll() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int toRemove : indexes) {
                this.ribbonBand.removeRibbonGalleryButtons(GALLERY_NAME, buttons[i][toRemove]);
                removed++;
    ...
    public void testRemovalAll3() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int toRemove : indexes) {
                this.ribbonBand.removeRibbonGalleryButtons(GALLERY_NAME, buttons[i][toRemove],
                        buttons[i][9 - toRemove], buttons[i + 2][toRemove],
                        buttons[i + 2][9 - toRemove]);
                removed += 4;
                assertEquals(this.gallery.getButtonCount(), 40 - removed);
            }
        }
